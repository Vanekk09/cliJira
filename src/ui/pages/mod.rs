...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let name_col = get_column_string(&epic.name, 11);
        let description_col = get_column_string(&epic.description, 24);
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let name_col = get_column_string(&story.name, 11);
        let description_col = get_column_string(&story.description, 24);
        let status_col = get_column_string(&story.status.to_string(), 15);
        println!("{} | {} | {} | {}", id_col, name_col, description_col, status_col);
