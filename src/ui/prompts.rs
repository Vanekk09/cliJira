...
fn create_epic_prompt() -> Epic {
    ...
    let epic = Epic::new(epic_name.trim().to_owned(), epic_description.to_owned());
...
fn create_story_prompt() -> Story {
    ...
    let story = Story::new(story_name.trim().to_owned(), story_description.to_owned());
...
fn delete_epic_prompt() -> bool {
    ...
    let delete_result = get_user_input();

    delete_result.trim().to_lowercase() == "y"
}
...
fn update_status_prompt() -> Option<Status> {
    ...
    let status = status_input.trim().parse::<u8>();
