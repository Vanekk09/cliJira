...
impl JiraDatabase {
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        db_state.stories.retain(|id, _| !epic.stories.contains(id));
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        let json_str = read_to_string(&self.file_path)
            .map_err(|e| anyhow!("Failed to read the file: {}", e))?;

        let db_state = from_str(&json_str)
            .map_err(|e| anyhow!("Failed to deserialize the string: {}", e))?;

        Ok(db_state)
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let json_str = to_string(&db_state)
            .map_err(|e| anyhow!("Failed to serialize the string: {}", e))?;

        write(&self.file_path, json_str)
            .map_err(|e| anyhow!("Failed to write the file: {}", e))?;
